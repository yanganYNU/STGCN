...
def search_data(sequence_length, num_of_batches, label_start_idx,
    ...
    x_idx = []
    # for i in range(1, 3):从1开始，一直到2不含边界
...
def get_sample_indices(data_sequence, num_of_weeks, num_of_days, num_of_hours,
    ...
    # print('week_sample.ndim=',week_sample.ndim)
    # print('week_sample.size=', week_sample.size)
    # print('week_sample.shape=', week_sample.shape)

    day_sample = np.concatenate([data_sequence[i: j] for i, j in day_indices], axis=0)
    # print('day_sample.ndim=',day_sample.ndim)
    # print('day_sample.size=', day_sample.size)
    # print('day_sample.shape=', day_sample.shape)

    hour_sample = np.concatenate([data_sequence[i: j] for i, j in hour_indices], axis=0)
    # print('hour_sample.ndim=',hour_sample.ndim)
    # print('hour_sample.size=', hour_sample.size)
...
def scaled_Laplacian(A):
    ...
    Parameters
    ----------
    adjacency_matrix: np.ndarray, shape is (N, N), N is the num of vertices
    ...
    out = (2 * L) / lambda_max - np.identity(A.shape[0])
    return out
